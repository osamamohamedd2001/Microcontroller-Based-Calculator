...
{
	uint32 Local_u32IntegerPart;
	f32 Local_f32DecimalPart;

	Local_u32IntegerPart = (uint32)Copy_f32Number;

	CLCD_voidSendNumber((sint32)Local_u32IntegerPart);

	CLCD_voidSendData('.');

	/* Subtract the integer part from the original number to get the decimal part */
	Local_f32DecimalPart = Copy_f32Number - (f32)Local_u32IntegerPart;

	Local_f32DecimalPart = Local_f32DecimalPart * 100.0;

	CLCD_voidSendNumber((sint32)Local_f32DecimalPart);
}
